...
import { encodeBmp, type CanvasBackend, type CanvasHandle, type Canvas2D } from '@placeholderer/core';
...
export const nodeCanvasBackend: CanvasBackend = {
  createCanvas(width, height) {
    ...
    return {
      ...
      encode: async (mime) => {
        // BMP isn't supported by @napi-rs/canvas's toBuffer, so we
        // read the RGBA pixel data and run our own encoder.
        if (mime === 'image/bmp') {
